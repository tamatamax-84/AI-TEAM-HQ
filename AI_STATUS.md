...
- `RETIRED` — intentionally removed from the active team and no longer considered for routing or review.
...
## Retired / removed
- Gemini — `RETIRED`. The user has explicitly requested removal because Gemini is no longer usable for this team. It must not be selected for routing, review, failover, or future work unless the user explicitly re-adds it.
...
- CodeRabbit is an evidence-verified Reviewer Slot candidate based on PR #1. Its verified role is automated low-load PR review only.
...
- A `RETIRED` AI is excluded from normal routing and reviewer selection until explicitly re-added by the user.
